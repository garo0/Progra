...
def filtro3(funcion, unaLista, n):
    if vacia(unaLista):
        return listaVacia 
    else:
        if (funcion(cabeza(unaLista),n)):
            return crearLista(cabeza(unaLista),filtro3(funcion, cola(unaLista),n))
        else:
            return filtro3(funcion,cola(unaLista),n)
        
# Tests
valores = lista(6, lista(4, lista(8, listaVacia)))
assert filtro3(lambda x,y: x < y, valores,5) == lista(4, listaVacia)


# filtro :  (Y X -> bool) lista(X) Y -> lista(X)
# devuelve lista con todos los valores donde operador devuelve True
def filtro2(funcion, unaLista):
    if vacia(unaLista):
        return listaVacia 
    else:
        if (funcion(cabeza(unaLista))):
            return crearLista(cabeza(unaLista),filtro2(funcion, cola(unaLista)))
        else:
            return filtro2(funcion,cola(unaLista))
        
# Tests
valores = lista(6, lista(4, lista(8, listaVacia)))
assert filtro2(lambda x: x < 5, valores) == lista(4, listaVacia)

# mapa : (X -> Y) lista(X) -> lista(float(x.numerador)/x.denominador < fraccY)
